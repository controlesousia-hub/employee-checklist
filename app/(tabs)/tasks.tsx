...
export default function TasksScreen() {
  ...
  // État de la fenêtre de création
  ...
  const handleAddTask = async () => {
    ...
    // Si l'insertion réussit, on déclenche l'appel vers Make.com pour l'e-mail
        // Si l'insertion réussit, notification via la passerelle serveur (Supabase → Make)
    if (!error) {
      const { error: notifyError } = await supabase.functions.invoke('notify-make', {
        body: {
          type: 'INSERT',
          record: {
            title: title.trim(),
      ...
      if (notifyError) {
        // Non bloquant : la tâche est déjà créée en base
        console.log('Erreur de notification :', notifyError.message);
      }
    }
